...
> ### ⚠️ 이 프롬프트를 쓰세요. URL만 전달하는 방식은 권장하지 않습니다.
>
> 재현 테스트 결과입니다.
>
> | 실행 방법 | 시도 | 정상 산출 |
...
> | **아래 프롬프트 사용** | 1회 | **1회** |
>
> URL만 주면 AI가 리포를 *"따라야 할 절차"*가 아니라 *"참고 자료"*로 다루는 경우가 있습니다.
> 그러면 택소노미를 자기 방식으로 새로 만들고, `priority_calc.py`를 실행하지 않고 점수를
> 직접 계산하며, 리포트 템플릿 대신 HTML을 새로 씁니다. **결과물은 그럴듯해 보이지만
> 이 도구의 산출이 아니며, 다시 돌려도 같은 결과가 나오지 않습니다.**
>
> 아래 프롬프트는 그 지시들을 사용자 메시지에 직접 담아 전달하므로 준수율이 높습니다.
> *(프롬프트 경로는 아직 1회만 검증했습니다. URL-only 대비 우위는 분명하나 표본은 작습니다.)*
...
| **정의에 맞지 않는 발화를 억지로 A~J에 배정** | 배정 전 그 대분류의 `guideline`을 **definition·exclude 한 쌍으로** 확인한다. 그 대분류의 `minor` 목록에 **담을 자리가 아예 없으면 그 대분류가 아니다 → `기타`**. 억지 배정은 Priority를 부풀리고 확장 마법사가 감지할 신호를 지운다. 단, **사유가 안 드러나서** 중분류를 비우는 것은 정상이며(규칙 R1) 이때는 기타가 아니다 |
| 중분류를 자유 서술로 작성 | `minor[].id`(`A1`·`C4`·`E2` 등)에서만 고른다. 새 이름을 만들거나 대분류명을 복사하지 마라. 없으면 `null` |
